...
class EFramework_Mega_Menu_Walker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        $this->item = $item->ct_onepage;
        $extra_menu_custom = apply_filters("ct_menu_edit", array());
        if ($this->item === 'is-one-page' && !wp_script_is('single-page-nav')) {
            wp_register_script('single-page-nav', CT_PATH . 'assets/js/lib/jquery.singlePageNav.js', array('jquery'), 'all', true);
        ...
        add_filter('nav_menu_link_attributes', function ($atts) {
            if (empty($atts['class'])) {
                $atts['class'] = trim($this->item);
            } elseif (!empty($this->item) && strpos($atts['class'], $this->item) === false) {
                $atts['class'] = trim($atts['class'] . ' ' . $this->item);
            }
            return $atts;
        });
