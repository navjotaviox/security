    ...
    function add_section($taxonomy, $section = array())
    {
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$taxonomy])) {
                trigger_error(sprintf(esc_html__('The field with id %1$s for taxonomy %2$s is already registered.', CT_TEXT_DOMAIN), esc_html($field['id'], CT_TEXT_DOMAIN), esc_html($taxonomy, CT_TEXT_DOMAIN)));
...
//            echo '<p>';
//            esc_html_e('All invalid custom field values for newly created term or existing term will not be saved, if you are missing something, please check field descriptions for instructions how to input valid values.', CT_TEXT_DOMAIN);
//            echo '</p>';
